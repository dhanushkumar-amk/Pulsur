...
use fundamentals::{LinkedList, SimpleHashMap, Stack, ThreadPool};
...
fn main() {
    ...
    println!(
        "Lab 1: LinkedList -> Peeked result: {:?}",
        list.peek_front()
    );
    ...
    println!(
        "Lab 4: Thread Pool -> Result after 4 jobs: {:?}",
        *result.lock().unwrap()
    );
